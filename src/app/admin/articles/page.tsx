...
export default function ArticlesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    let q = supabase.from("articles").select("id,title,category,required_tier,status,view_count,created_at", { count: "exact" });
  ...
  const handleBatchDelete = async () => {
    ...
    await supabase.from("articles").delete().in("id", Array.from(selected));
    setSelected(new Set());
    load();
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[#64748B]">共 {total} 篇</p>
        </div>
        <LinkNoPrefetch href="/admin/articles/new" className="flex items-center gap-2 px-4 py-2.5 bg-[#06B6D4] text-white text-sm font-semibold rounded-xl hover:bg-[#0891B2]">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} placeholder="搜索文章…" className="w-full pl-9 pr-3 py-2 bg-[#1E293B] border border-[rgba(30,41,59,0.6)] rounded-lg text-sm text-[#F1F5F9] placeholder-[#64748B] outline-none focus:border-[#06B6D4]" />
        </div>
        <div className="flex gap-1">
          {STATUSES.map(s => (
            <button key={s} onClick={() => { setStatus(s); setPage(0); }} className={`px-3 py-1.5 text-xs rounded-lg transition-all ${status === s ? "bg-[#06B6D4]/20 text-[#06B6D4] border border-[#06B6D4]/30" : "bg-[#1E293B] text-[#94A3B8] border border-[rgba(30,41,59,0.4)] hover:text-[#F1F5F9]"}`}>
              {s === "全部" ? "全部" : s === "published" ? "已发布" : s === "draft" ? "草稿" : "定时"}
            </button>
          ))}
        </div>
        {selected.size > 0 && (
          <button onClick={handleBatchDelete} className="flex items-center gap-1 px-3 py-1.5 text-xs bg-[#EF4444]/10 text-[#EF4444] border border-[#EF4444]/20 rounded-lg hover:bg-[#EF4444]/20">
            <Trash2 className="w-3 h-3" /> 删除 {selected.size} 篇
          </button>
        )}
      </div>
      ...
      <div className="glass-card overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-sm text-[#64748B]">暂无文章</div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-[#0F172A]/60">
              <tr className="text-left text-xs text-[#64748B]">
                <th className="p-3 w-10"><input type="checkbox" onChange={e => setSelected(e.target.checked ? new Set(articles.map(a => a.id)) : new Set())} checked={selected.size === articles.length && articles.length > 0} /></th>
                <th className="p-3">标题</th>
                <th className="p-3 hidden md:table-cell">分类</th>
                <th className="p-3 hidden md:table-cell">等级</th>
            ...
            <tbody>
              {articles.map(a => (
                <tr key={a.id} className="border-t border-[rgba(30,41,59,0.3)] hover:bg-[#1E293B]/30">
                  <td className="p-3"><input type="checkbox" checked={selected.has(a.id)} onChange={() => toggleSelect(a.id)} /></td>
                  <td className="p-3 text-[#F1F5F9] truncate max-w-[200px]">{a.title}</td>
                  ...
                  <td className="p-3">
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => handleDelete(a.id)} className="p-1.5 text-[#64748B] hover:text-[#EF4444]"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
