    ...
    <div className="pt-20 pb-20">
      <div className="max-w-4xl mx-auto px-4 md:px-6">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <LinkNoPrefetch href="/forum/new" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#06B6D4] text-white text-sm font-medium rounded-xl hover:bg-[#0891B2]"><Plus className="w-4 h-4" />发帖</LinkNoPrefetch>
        </div>

        {loading ? <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="h-14 rounded-xl bg-[#1E293B]/20 animate-pulse" />)}</div> : (
          <div className="glass-card overflow-hidden">
            {posts.map((post, i) => (
              <LinkNoPrefetch href={"/forum/post/?id=" + post.id} key={post.id} className={"p-4 flex items-center gap-4 border-b border-[rgba(30,41,59,0.3)] last:border-0 hover:bg-[#1E293B]/20 transition-colors cursor-pointer" + (post.is_pinned ? " bg-[#F59E0B]/3" : "")}>
